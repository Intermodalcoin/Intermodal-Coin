...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeIntermodalCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIntermodalCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeIntermodalCoinAmount = strAnonymizeIntermodalCoinAmount.remove(strAnonymizeIntermodalCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIntermodalCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeIntermodalCoinAmount*COIN) nMaxToAnonymize = nAnonymizeIntermodalCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeIntermodalCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeIntermodalCoinAmount));
        strAnonymizeIntermodalCoinAmount = strAnonymizeIntermodalCoinAmount.remove(strAnonymizeIntermodalCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIntermodalCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeIntermodalCoinAmount == 0){
