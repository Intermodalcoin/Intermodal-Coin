...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeIntermodalCoinAmount"))
        settings.setValue("nAnonymizeIntermodalCoinAmount", 1000);
    nAnonymizeIntermodalCoinAmount = settings.value("nAnonymizeIntermodalCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeIntermodalCoinAmount"))
        SoftSetArg("-anonymizeintermodalcoinamount", settings.value("nAnonymizeIntermodalCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeIntermodalCoinAmount:
            return QVariant(nAnonymizeIntermodalCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeIntermodalCoinAmount:
            nAnonymizeIntermodalCoinAmount = value.toInt();
            settings.setValue("nAnonymizeIntermodalCoinAmount", nAnonymizeIntermodalCoinAmount);
            emit AnonymizeIntermodalCoinAmountChanged(nAnonymizeIntermodalCoinAmount);
            break;
        default:
            break;
        }
