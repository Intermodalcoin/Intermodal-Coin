...
void Shutdown()
{
    ...
    RenameThread("intermodalcoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: intermodalcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: intermodalcoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 11707)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 11708)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeintermodalcoinamount=<n> " + _("Keep N IntermodalCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. IntermodalCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. IntermodalCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("IntermodalCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "IntermodalCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of IntermodalCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart IntermodalCoin to complete") << "\n";
    ...
    nAnonymizeIntermodalCoinAmount = GetArg("-anonymizeintermodalcoinamount", 0);
    if(nAnonymizeIntermodalCoinAmount > 999999) nAnonymizeIntermodalCoinAmount = 999999;
    if(nAnonymizeIntermodalCoinAmount < 2) nAnonymizeIntermodalCoinAmount = 2;
    ...
    LogPrintf("Anonymize IntermodalCoin Amount %d\n", nAnonymizeIntermodalCoinAmount);
