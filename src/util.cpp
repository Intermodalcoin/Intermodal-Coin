...
int nAnonymizeIntermodalCoinAmount = 1000;
...
#else
    const char* pszModule = "intermodalcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IntermodalCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IntermodalCoin
    // Mac: ~/Library/Application Support/IntermodalCoin
    // Unix: ~/.intermodalcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IntermodalCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "intermodalcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "intermodalcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong IMCOIN will not work properly.");
